...
# ✅ Max tokens config — saves API usage significantly
...
SCORE_CONFIG = genai.types.GenerationConfig(
    max_output_tokens=10,  # Only needs one word: Hot/Warm/Cold
    temperature=0.1
)
...
4. CRITICAL — LANGUAGE SWITCHING: If the lead at any point says they prefer a different language (e.g., "please speak in English", "only English please", "Hindi mein baat karo"), you MUST immediately and permanently switch to that language for every single response after that. Never go back unless they ask again.
"""
...
def build_lang_instruction(language: str) -> str:
    ...
    if language == 'Hindi':
        return """
LANGUAGE RULE — ABSOLUTE: Respond ONLY in Hindi. Use Romanized Hindi (Roman script, NOT Devanagari).
Example: "Bilkul ji! Rupeezy ke saath aap 100% brokerage kama sakte hain, aur paise roz milte hain. Aapke paas kitne clients hain abhi?"
NEVER use English words unless they are financial terms (brokerage, payout, SEBI, portal, Rupeezy, RISE Portal).
If the lead writes in English or Hinglish, still respond in Romanized Hindi.
"""
...
def get_lead_score(history):
    try:
        model = genai.GenerativeModel("gemini-2.5-flash")  # ✅ Use Flash for scoring (cheaper)
        prompt = f"Analyze this conversation and classify the lead as 'Hot', 'Warm', or 'Cold'. Return only the word.\n\nHistory: {history}"
        response = model.generate_content(prompt, generation_config=SCORE_CONFIG)  # ✅ max_tokens added
...
def chat():
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:  # ✅ Removed hardcoded old key check
    ...
    try:
        # Detect if user is requesting a language switch mid-conversation
        ...
        model = genai.GenerativeModel(
            model_name="gemini-2.5-flash",  # ✅ Changed to Flash — much cheaper, still great
        ...
        response = chat_session.send_message(
            user_message,
            generation_config=GENERATION_CONFIG  # ✅ max_tokens applied
        )
        ...
        return jsonify({
            ...
            "score": score,
            "mood": mood,
            "resolved_language": language  # lets frontend sync when user switches mid-conversation
        })
...
def generate_summary():
    ...
    try:
        model = genai.GenerativeModel("gemini-2.5-flash")  # ✅ Flash for summary too
        summary_prompt = f"""
        ...
        response = model.generate_content(
            summary_prompt,
            generation_config=SUMMARY_CONFIG  # ✅ max_tokens applied
        )
...
def text_to_speech():
    """Convert text to speech using gTTS — natural Indian voice for Hindi/Hinglish/English."""
    ...
    # Strip markdown/emoji/special chars that TTS mispronounces
    import re
    ...
    try:
        if language == 'Hindi':
            # Native Hindi voice — excellent pronunciation
            tts = gTTS(text=text, lang='hi', slow=False)
        elif language == 'Hinglish':
            # Hinglish is Roman script mix of Hindi+English — Indian English voice handles it naturally
            tts = gTTS(text=text, lang='en', tld='co.in', slow=False)
        else:
            # English with Indian accent (tld='co.in' = Indian English)
            tts = gTTS(text=text, lang='en', tld='co.in', slow=False)
